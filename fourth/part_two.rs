...
impl Guard {
    ...
    fn most_slept_frequency(&self) -> u32 {
        let mut slept_vec: Vec<_> = self.sleeping_minutes.iter().collect();
        slept_vec.sort_by(|a, b| a.1.cmp(&b.1).reverse());
...
fn main() -> io::Result<()> {
    ...
    guards_vec.sort_by(|a, b| a.most_slept_frequency().cmp(&b.most_slept_frequency()).reverse());

    let guard = guards_vec[0];
    let result = guard.id * guard.most_slept_minute();
    println!("Mr Sleepyhead is guard id {} and has slept {} minutes his most slept minute is {} when he slept {} times", guard.id, guard.total_sleeping_minutes, guard.most_slept_minute(), guard.most_slept_frequency());
